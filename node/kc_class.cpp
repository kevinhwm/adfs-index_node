...
void kc_class::init( string dbpath, int node_num,  char cache_mode,long cache_size,long kchfile_size,int max_node_count,int init_mode) //init the kc_class
{
     
/*------------+--init indexdb-------------------*/
    kc_class::m_node_num=node_num;
    kc_class::m_dbpath=dbpath;
    //kc_class.split_node=split_node;
    ...
    //cout<<kc_class::m_max_node_count<<endl;
...
/*--------------init nodedb----------------------*/
    ...
    {
        m_node_num+=1;
        //cout<<m_node_num<<endl;
        //cout<<m_kchfile_num<<endl;
        //cout<<node_num<<endl;
        //cout<<dbnum<<endl;
        //cout<<"node_num"<<kt_status["node_num"]<<endl;
        //cout<<kt_status["node_num"]<<endl;
        fenku_init();
    }
    /*
...
//   cout<<"init datebase sucess"<<endl;
}
    ...
    if (m_cache_mode=='y')
    {
        ...
        if( m_cachedb.status( &statusinfo ) )
    	{
    		...
    		for( std::map<std::string, std::string >::iterator it = statusinfo.begin();
    			...
    			it++ )
    		{
    			//evbuffer_add_printf( returnbuffer, "%s=%s\n", it->first.c_str(), it->second.c_str() );
...
void kc_class::fenku_init()  //fenku init 
{
     ...
     if (m_max_node_count>m_node_num-1)
     {
         for (int i=m_node_num-1;i<=m_node_num;i++)
         {
    	    ...
    	    //sprintf(tempb,"%d",i);
        	//nodepath=kt_status["node_path"]+"/"+tempb+".kch";
        	if (i<m_node_num)
        	{
        	m_vHashDBList.push_back(InitHashDB(nodepath,i,"ro"));
            }
    ...
    //cout<<m_kchfile_num<<endl;
    m_asi_num=1;
    //cout<<kchfile_num<<"fenku sucess"<<endl;
}
...
int kc_class::set(const char * upname, const int upnamelen, const char * updata, const long updatelen )    //upload file function
{
     ...
     else
     {
        ...
        // cout<<"temp_upname"<<temp_upname<<endl;
         if (strlen(temp_upname)>130)
            return -2;
         //string  temp_upname=upname;
         //temp_upname+=".";
         //temp_upname+=time_combine();
         sprintf(tempindex,"%d=%s|%.900s\0",m_node_num,temp_upname,pIndexBuffer);
         //cout<<"tempindex "<<tempindex<<endl;
         //cout<<kt_status["node_num"]<<endl;
         //tempindex=kt_status["node_num"]+"="+temp_upname+"|"+pIndexBuffer;
     ...
     //m_indexdb.set( upname, upnamelen, (const char *)kt_status["nodeRW"].c_str(), kt_status["nodeRW"].size() );
     now =time(0);
     tnow = localtime(&now);
...
int kc_class::get(const char *pname, char * pFileBuffer, const int MaxBufferLength)   //download file function
{
	...
	else if( -1 == len )
	{
        ...
        //cout<<"pIndexBuffer "<<pIndexBuffer<<endl;
        //cout<<"tempindex "<<tempindex<<endl;
...
int kc_class::remove(const char *pname) //delete file function
{
     ...
     else
     {
         ...
         //cout<<"pnameValue begin"<<endl;
         //cout<<"pIndexBuffer1 "<<pIndexBuffer<<endl;
         strcpy(pnameValue,pIndexBuffer);
         //cout<<"pnameValue "<<pnameValue<<endl;
...
//        cout<<"indexPname "<<indexPname<<endl;
